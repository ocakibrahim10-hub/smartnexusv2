      ...
      <div className="flex-1 p-6 space-y-6">
        ...
        {isDealer && (
          ...
          <div className="flex flex-wrap gap-2 mb-2">
            <a href={`/${user?.panel || 'bayi'}/businesses`} className="text-xs px-3 py-1.5 rounded-lg bg-[#E0E0FF] text-[#3944B8] font-medium hover:opacity-90">
              + Yeni İşletme
            </a>
            <a href={`/${user?.panel || 'bayi'}/dealer/reports`} className="text-xs px-3 py-1.5 rounded-lg border border-[#EFEDF4] text-gray-600 font-medium hover:bg-white">
              Raporlar
            </a>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
          ...
          {(safeNum(kpis.pendingPaymentCount) > 0 || (data.pendingPaymentBusinesses ?? []).length > 0) && (
            <div className="card border-l-4 border-l-amber-500 p-4">
              <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
                <div>
                  <h3 className="font-semibold text-gray-900 flex items-center gap-2">
                    <AlertTriangle size={18} className="text-amber-600" />
                    Ödeme Bekleyen İşletmeler
                  </h3>
                  <p className="text-xs text-gray-500 mt-0.5">
                    {safeNum(kpis.pendingPaymentCount)} işletme — Sanal POS ödemesi tamamlanmadı
                  </p>
                </div>
                <a
                  href={`/${user?.panel || 'bayi'}/subscriptions?filter=pending`}
                  className="text-xs font-medium text-[#606BDF] hover:underline flex items-center gap-1"
                >
                  Tümünü gör <ArrowUpRight size={14} />
                </a>
              </div>
              <ul className="space-y-2">
                {(data.pendingPaymentBusinesses ?? []).map((b: any) => (
                  <li
                    key={b.id}
                    className="flex items-center justify-between py-2 px-3 rounded-xl bg-amber-50/60 border border-amber-100"
                  >
                    <div>
                    ...
                    <a
                      href={`/${user?.panel || 'bayi'}/subscribe?tenantId=${b.id}&plan=${b.plan}`}
                      className="text-xs px-3 py-1.5 rounded-lg bg-[#606BDF] text-white font-medium hover:opacity-90"
                    >
                      Ödemeye Git
                    </a>
                  </li>
                ...
                {[
                  ...
                  <div key={s.label}>
                    ...
                    <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden">
                        <div
                          className={`h-full ${s.color} rounded-full pct-bar`}
