...
OUT_PATH = Path(__file__).resolve().parent / "SmartNexus_Guncel_Durum_Raporu_2026.pptx"
...
def build_presentation() -> Presentation:
    ...
    tp.text = "SmartNexus Güncel Durum Raporu"
    tp.font.size = Pt(40)
    tp.font.bold = True
    ...
    add_kpi_card(s, Inches(0.6), Inches(1.35), Inches(2.9), Inches(1.15), "Türkiye geneli sıra", "~18 / 35", accent=True)
    add_kpi_card(s, Inches(3.7), Inches(1.35), Inches(2.9), Inches(1.15), "Ağırlıklı puan", "64 / 100")
    add_kpi_card(s, Inches(6.8), Inches(1.35), Inches(2.9), Inches(1.15), "Niş segment sırası", "2–4 / 8")
    add_kpi_card(s, Inches(9.9), Inches(1.35), Inches(2.8), Inches(1.15), "Ürün tamamlanma", "~%80")
    add_bullet_box(s, Inches(0.6), Inches(2.75), Inches(12), Inches(3.8), [
        ...
        "Güçlü yönler: Canlı Prod (Vercel & Render devrede), PayTR Entegrasyonu, Çoklu Şube & Transfer, modern UX (%88).",
        "Kritik açıklar: Gerçek GİB e-fatura (%18), pazaryeri mock (%20), genel muhasebe yok.",
        "12 ay hedef: sıra 10–12, puan 72–75, 50–100 ödeme yapan müşteri, ₺75K–150K MRR.",
    ], font_size=13)
    add_footer(s)
    ...
    feat_rows = [
        ...
        ["Stok & Depo", "%90", "%90", "%88", "%45", "%95"],
        ["POS", "%73", "%85", "%80", "%20", "%98"],
        ["E-Fatura (GİB)", "%18", "%95", "%90", "%95", "%88"],
        ["Pazaryeri", "%20", "%60", "%50", "%55", "%90"],
        ["Çoklu Şube", "%95", "%90", "%75", "%10", "%95"],
        ["Bayi SaaS", "%85", "%40", "%35", "%5", "%50"],
        ["UX Modernlik", "%88", "%55", "%60", "%85", "%65"],
        ["Ağırlıklı Ort.", "%64", "%83", "%78", "%58", "%84"],
    ]
    ...
    int_rows = [
        ...
        ["Ödeme Gateway", "PayTR (Tamamlandı)", "B2B gerekli", "%100 (TAMAM)"],
    ...
    score_rows = [
        ...
        ["Stok & WMS", "90", "82", "+8"],
        ["Satış & POS", "73", "75", "-2"],
        ["TMS (Lojistik)", "85", "65", "+20"],
        ["B2B & e-ticaret", "65", "70", "-5"],
        ["E-dönüşüm", "18", "88", "-70"],
        ["Çoklu şube / bayi", "95", "55", "+40"],
        ["Fiyat / performans", "85", "60", "+25"],
        ["UX & modernlik", "88", "68", "+20"],
        ["Pazar olgunluğu", "15", "85", "-70"],
        ["AĞIRLIKLI TOPLAM", "64", "74", "-10"],
    ]
    ...
    swot_data = [
        ("Güçlü Yönler (S)", SUCCESS, [
            ...
            "Modern SaaS UX (%88)",
            "Kusursuz Çoklu Şube & Transfer Mimarisi",
            "Canlı Prod (Vercel & Render Devrede)",
            "PayTR Ödeme Gateway Entegre",
        ]),
        ("Zayıf Yönler (W)", DANGER, [
            "Gerçek GİB e-fatura yok",
            "Genel muhasebe / beyanname yok",
            "Pazaryeri faturalama mock",
            "Henüz fatura kesen canlı müşteri yok",
    ...
    phases = [
        ("Faz 1 · TAMAMLANANLAR", "Temel eksikler giderildi", [
            "(TAMAMLANDI) Production API + DB + Vercel",
            "(TAMAMLANDI) Şube Stok Transfer & TMS",
            "(TAMAMLANDI) PayTR Ödeme Entegrasyonu",
            "(TAMAMLANDI) Rol bazlı yetki (RolesGuard)",
        ]),
        ("Faz 2 · 0-3 Ay", "Canlıya çıkış blokörleri", [
            "GİB e-fatura entegratörü (İzibiz/Uyumsoft)",
            "Banka entegrasyonu (Açık Bankacılık)",
            "Mobil Uygulama Stabilizasyonu",
            "Sistem içi kılavuzlar (Onboarding)",
        ]),
        ("Faz 3 · 3–6 Ay", "Farklılaşma", [
    ...
    targets = [
        ("Türkiye sırası", "18", "12–15"),
        ("Ağırlıklı puan", "64", "75+"),
    ...
    cp.text = (
        "Sonuç: Vercel/Render Production, PayTR ve Şube Transferleri canlıya alındı. \n"
        "E-dönüşüm ve Açık Bankacılık ile ticari satışa tamamen hazır hale gelinecek."
    )
    cp.font.size = Pt(14)
